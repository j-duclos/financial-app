...
EXPOSE 8000
CMD ["sh", "-c", "python manage.py migrate --no-input && exec gunicorn config.wsgi:application --bind 0.0.0.0:${PORT:-8000} --timeout ${GUNICORN_TIMEOUT:-240} --workers ${GUNICORN_WORKERS:-2} --threads ${GUNICORN_THREADS:-4}"]
