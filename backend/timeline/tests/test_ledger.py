...
class TestBuildTimeline:
    ...
    def _scheduled_transfer_pair(
        ...
        *,
        ...
        """Autopay-style transfer pair: no rule, both legs PLANNED and source=ACTUAL."""
        tg = TransferGroup.objects.create(
            household=household,
        ...
        return tg, out_leg, in_leg

    def test_autopay_transfer_pair_hidden_when_card_paid_off(self, user, household, db):
        """Rule-less autopay pair to a clear card disappears from both views but is not deleted."""
        from datetime import timedelta
        ...
        tg, out_leg, in_leg = self._scheduled_transfer_pair(
            household,
            from_account=bank,
            to_account=card,
            amount="1.00",
        ...
        for kwargs in ({"account_id": bank.id}, {}):
            rows = build_timeline(user, start, end, **kwargs)
            leaked = [r for r in rows if r.get("transaction_id") in (out_leg.pk, in_leg.pk)]
            assert leaked == [], f"expected autopay pair hidden for {kwargs}; got {leaked}"

        # Hide-only: nothing regenerates these rows, so they must survive in the database.
        assert Transaction.objects.filter(pk__in=(out_leg.pk, in_leg.pk)).count() == 2
        assert TransferGroup.objects.filter(pk=tg.pk).exists()

    def test_autopay_transfer_pair_kept_when_card_still_owes(self, user, household, db):
        from datetime import timedelta
        ...
        _tg, out_leg, _in_leg = self._scheduled_transfer_pair(
            household,
            from_account=bank,
            to_account=card,
            amount="50.00",
        ...
        rows = build_timeline(user, start, end, account_id=bank.id)
        assert [r for r in rows if r.get("transaction_id") == out_leg.pk]

    def test_autopay_transfer_pair_kept_when_a_leg_is_reconciled(self, user, household, db):
        """A reconciled leg means the payment really happened — the group stays visible."""
        from datetime import timedelta
        ...
        _tg, out_leg, in_leg = self._scheduled_transfer_pair(
            household,
            from_account=bank,
            to_account=card,
            amount="1.00",
        ...
        in_leg.reconciled = True
        in_leg.save(update_fields=["reconciled"])

        rows = build_timeline(user, start, end, account_id=bank.id)
        assert [r for r in rows if r.get("transaction_id") == out_leg.pk]

    def test_scheduled_savings_transfer_is_never_suppressed(self, user, household, db):
        """Only debt destinations are suppressed — a savings transfer must always show."""
        from datetime import timedelta
