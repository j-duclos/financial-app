...
def rematch_unmatched_manual_actuals(*, account_id: int | None = None) -> int:
    ...
    if account_id is not None:
        qs = qs.filter(account_id=account_id)
    pending_imports = Transaction.objects.filter(
        source=Transaction.Source.PLAID,
        import_match_status__in=PENDING_PLAID_IMPORT_STATUSES,
    ).exclude(plaid_transaction_id__isnull=True).exclude(plaid_transaction_id="")
    if account_id is not None:
        pending_imports = pending_imports.filter(account_id=account_id)
    if not pending_imports.exists():
        return 0
    matched = 0
