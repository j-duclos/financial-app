...
/** Mobile-friendly summary line. */
export function actionCenterMobileSummaryText(summary: ActionCenterSummary): string {
  const parts = [`${summary.total} item${summary.total === 1 ? "" : "s"} need attention`];
  if (summary.critical > 0) parts.push(`${summary.critical} critical`);
  if (summary.atRisk > 0) parts.push(`${summary.atRisk} at risk`);
  if (summary.watch > 0) parts.push(`${summary.watch} watch`);
  return parts.join(" · ");
