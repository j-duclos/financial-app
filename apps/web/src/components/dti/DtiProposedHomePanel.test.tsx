...
const purchaseInput: DtiProposedPurchaseInput = {
...
const dollarEstimate: DtiPurchaseEstimateResult = {
  ...purchaseEstimate,
  down_payment_type: "dollars",
  down_payment_value: "14000.00",
};
...
function Harness({
  ...
  const [selectedMode, setSelectedMode] = useState<"monthly_payment" | "purchase">(
    applied?.mode ?? "monthly_payment"
  );
  return (
...
describe("DtiProposedHomePanel", () => {
  ...
  it("shows both named modes and monthly fields labeled Monthly", () => {
    ...
    expect(screen.getByText(/Do not enter the home price or loan balance here/)).toBeInTheDocument();
  });
  ...
  it("displays backend purchase results including converted percent down payment", () => {
    render(
      <Harness
        applied={{ mode: "purchase", purchase: purchaseInput }}
  ...
  it("displays the backend down-payment percentage for a dollar down payment", () => {
    render(
      <Harness
        applied={{
          mode: "purchase",
          purchase: { ...purchaseInput, down_payment_type: "dollars", down_payment_value: "14000.00" },
        }}
        purchaseResult={dollarEstimate}
      />
    );
    expect(screen.getByTestId("dti-down-payment-converted")).toHaveTextContent("14,000.00");
    expect(screen.getByTestId("dti-down-payment-converted")).toHaveTextContent("3.50%");
