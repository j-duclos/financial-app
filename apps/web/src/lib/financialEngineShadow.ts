...
export function getWebFinancialEngineMode(): FinancialEngineMode {
  return parseFinancialEngineMode(
    import.meta.env.VITE_FINANCIAL_ENGINE_MODE,
    import.meta.env.VITE_FINANCIAL_ENGINE_SHADOW
  );
}

export function isWebFinancialEngineShadowEnabled(): boolean {
  return getWebFinancialEngineMode() !== "server";
}
...
function resolveMode(options?: { mode?: FinancialEngineMode; enabled?: boolean }): FinancialEngineMode {
  ...
  return getWebFinancialEngineMode();
}
