...
export default function Profile() {
  ...
  const { data: accountsData } = useQuery({
    ...
    queryFn: () => listAccounts({ household: householdNum!, page_size: 200 }),
  ...
  const inputClass =
    "mt-1 block w-full max-w-md rounded border border-gray-300 px-3 py-2 text-sm bg-white";
      ...
      <section className="bg-white rounded-lg border border-gray-200 shadow-sm p-6 space-y-4">
        ...
        <form onSubmit={handleSaveProfile} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={profile.username} disabled className={`${inputClass} bg-gray-50 text-gray-600`} />
          </div>
          <div>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">
              Used when connecting banks via Plaid (SMS verification). Saved as E.164 on the server.
            </p>
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Default account (optional)</label>
            <select
