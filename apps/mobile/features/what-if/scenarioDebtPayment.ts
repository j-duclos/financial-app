...
export function isDebtPaymentAccount(account: Account): boolean {
  return account.account_type === "CREDIT" || account.role === "loan";
}

export function balanceOwed(account: Account): number | null {
  const raw = account.balance_owed ?? account.balance;
  if (raw == null || String(raw).trim() === "") return null;
  const n = parseFloat(String(raw));
  return Number.isNaN(n) ? null : n;
}

function creditLimitAmount(account: Account): number | null {
  const raw = account.credit_limit;
  if (raw == null || String(raw).trim() === "") return null;
  const n = parseFloat(String(raw));
  return Number.isNaN(n) ? null : n;
}
